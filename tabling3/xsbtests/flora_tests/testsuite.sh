...
##
## $Id: testsuite.sh,v 1.1 2002-08-17 20:40:14 kifer Exp $
## 
##
...
#        path         -- full path name of the FLORA installation directory
...
if test -z "$FLORADIR" ; then
   echo "***FLORA installation directory hasn't been specified."
   FLORADIR=$testdir/../flora2
   echo "***Assuming $FLORADIR"
fi
if test ! -d "$FLORADIR" ; then
   echo "$FLORADIR: FLORA installation directory doesn't exist or has wrong permissions"
   exit 1
fi
...
if test $# -gt 1; then
    ...
    echo "       path      -- full path name of the FLORA installation directory"
    exit
fi
...
./testall.sh -opts "$options" -exclude "$excluded_tests" \
                              -add "$added_tests" -only "$only_tests" \
