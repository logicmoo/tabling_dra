...
:- ensure_loaded( library( lists ) ). % A YAP library, for reverse/2.
:- ensure_loaded( library( terms ) ). % A YAP library, for term_variables/2.
...
%% recordz( + term, + term ):
%% recorded( + term, + term ):
%% Yap does not have the versions that ignore the reference.
