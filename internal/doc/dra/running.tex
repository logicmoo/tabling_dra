...
as a directive; when you type in a term of the form \prog{?-~\patt{...}.} it
will be treated as a query; when you type in a term that does not begin with
\prog{:-} or \prog{?-}, it will also be treated as a query.
...
printout with statistics\index{statistics} accummulated since the previous
...
loading a program with \prog{prog(~\patt{filename}~)} have different
